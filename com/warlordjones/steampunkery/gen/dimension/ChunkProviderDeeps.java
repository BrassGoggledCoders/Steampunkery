...
public class ChunkProviderDeeps implements IChunkProvider {
	...
	for (int k = 0; k < 16; ++k) {
	    for (int l = 0; l < 16; ++l) {
		boolean flag = this.slowsandNoise[k + l * 16]
			+ this.hellRNG.nextDouble() * 0.2D > 0.0D;
		boolean flag1 = this.gravelNoise[k + l * 16]
			+ this.hellRNG.nextDouble() * 0.2D > 0.0D;
		int i1 = (int) (this.netherrackExclusivityNoise[k + l * 16] / 3.0D + 3.0D + this.hellRNG
			.nextDouble() * 0.25D);
		int j1 = -1;
	...
	this.netherCaveGenerator.generate(this, this.worldObj, par1, par2,
		abyte);
	...
	for (i2 = 0; i2 < par6; ++i2) {
	    adouble1[i2] = Math.cos((double) i2 * Math.PI * 6.0D
		    / (double) par6) * 2.0D;
	    double d2 = (double) i2;
	...
	for (i1 = 0; doGen && i1 < 8; ++i1) {
	    ...
	    (new WorldGenHellLava(Block.lavaMoving.blockID, false)).generate(
		    this.worldObj, this.hellRNG, j1, k1, l1);
	}
	...
	for (j1 = 0; doGen && j1 < i1; ++j1) {
	    ...
	    (new WorldGenFire()).generate(this.worldObj, this.hellRNG, k1, l1,
		    i2);
	}
	...
	if (doGen && this.hellRNG.nextInt(1) == 0) {
	    ...
	    k1 = this.hellRNG.nextInt(128);
	    l1 = l + this.hellRNG.nextInt(16) + 8;
	    (new WorldGenFlowers(Block.mushroomRed.blockID)).generate(
		    this.worldObj, this.hellRNG, j1, k1, l1);
	}
	...
	for (k1 = 0; k1 < 16; ++k1) {
	    ...
	    (new WorldGenHellLava(Block.lavaMoving.blockID, true)).generate(
		    this.worldObj, this.hellRNG, l1, i2, j2);
	}
	...
	if (par1EnumCreatureType == EnumCreatureType.monster) {
	    ...
	    if (this.genNetherBridge.func_142038_b(par2, par3, par4)
		    && this.worldObj.getBlockId(par2, par3 - 1, par4) == Block.netherBrick.blockID) {
		return this.genNetherBridge.getSpawnList();
	    }
	}
	...
	this.genNetherBridge.generate(this, this.worldObj, par1, par2,
		(byte[]) null);
    }
}
