...
public class TeleporterEther extends Teleporter {
    ...
    public TeleporterEther(final WorldServer par1WorldServer) {
	...
	for (i2 = i - b0; i2 <= i + b0; ++i2) {
		...
		label274: for (k2 = worldServerInstance.getActualHeight() - 1; k2 >= 0; --k2)
	...
	if (d0 < 0.0D)
		...
		for (j2 = k - b0; j2 <= k + b0; ++j2) {
		    d2 = j2 + 0.5D - par1Entity.posZ;
		    label222: for (k2 = worldServerInstance.getActualHeight() - 1; k2 >= 0; --k2)
			if (worldServerInstance.isAirBlock(i2, k2, j2)) {
			    while (k2 > 0
	...
	if (d0 < 0.0D) {
		...
		for (i3 = 1; i3 < 3; ++i3)
			...
			worldServerInstance.setBlock(k3, j3, i4,
				flag ? SteamBlocks.brass_block.blockID : 0);
		    }
	}
	for (k2 = 0; k2 < 4; ++k2) {
	    for (i3 = 0; i3 < 4; ++i3)
		for (l2 = -1; l2 < 4; ++l2) {
		    ...
		    /** change these blocks **/
		    worldServerInstance.setBlock(k3, j3, i4,
			    flag ? SteamBlocks.brass_block.blockID
		...
		for (l2 = -1; l2 < 4; ++l2) {
		    ...
		    worldServerInstance.notifyBlocksOfNeighborChange(k3, j3,
			    i4, worldServerInstance.getBlockId(k3, j3, i4));
		}
	}
	return true;
    }
     ...
     */
    @Override
    public boolean placeInExistingPortal(final Entity par1Entity,
	...
	final int i1 = MathHelper.floor_double(par1Entity.posZ);
	final long j1 = ChunkCoordIntPair.chunkXZ2Int(l, i1);
	boolean flag = true;
	double d4;
	int k1;
	if (destinationCoordinateCache.containsItem(j1)) {
	    final PortalPosition portalposition = (PortalPosition) destinationCoordinateCache
		    .getValueByKey(j1);
	    d3 = 0.0D;
		...
		final double d5 = k1 + 0.5D - par1Entity.posX;
		for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
		    final double d6 = l1 + 0.5D - par1Entity.posZ;
		    for (int i2 = worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2)
			/** change this block **/
			if (worldServerInstance.getBlockId(k1, i2, l1) == SteamBlocks.ether_portal.blockID) {
			    /** change this block **/
			    while (worldServerInstance.getBlockId(k1, i2 - 1,
				    l1) == SteamBlocks.ether_portal.blockID)
				--i2;
	...
	if (d3 >= 0.0D) {
		...
		destinationCoordinateCache
			.add(j1, new EtherPortalPosition(this, i, j, k,
				worldServerInstance.getTotalWorldTime()));
		destinationCoordinateKeys.add(Long.valueOf(j1));
	    }
	    double d8 = i + 0.5D;
	    final double d9 = j + 0.5D;
	    d4 = k + 0.5D;
	    int j2 = -1;
	    /** change this block **/
	    if (worldServerInstance.getBlockId(i - 1, j, k) == SteamBlocks.ether_portal.blockID)
		j2 = 2;
	    /** change this block **/
	    if (worldServerInstance.getBlockId(i + 1, j, k) == SteamBlocks.ether_portal.blockID)
		j2 = 0;
	    /** change this block **/
	    if (worldServerInstance.getBlockId(i, j, k - 1) == SteamBlocks.ether_portal.blockID)
		j2 = 3;
	    /** change this block **/
	    if (worldServerInstance.getBlockId(i, j, k + 1) == SteamBlocks.ether_portal.blockID)
		j2 = 1;
	    final int k2 = par1Entity.getTeleportDirection();
	    if (j2 > -1) {
     ...
     */
    @Override
    public void placeInPortal(final Entity par1Entity, final double par2,
	...
	if (par1 % 100L == 0L) {
	    final Iterator iterator = destinationCoordinateKeys.iterator();
