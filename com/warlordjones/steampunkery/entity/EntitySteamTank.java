...
public class EntitySteamTank extends Entity {
    ...
    public EntitySteamTank(final World par1World) {
	super(par1World);
	field_70279_a = true;
    ...
    public EntitySteamTank(final World par1World, final double par2,
     ...
     * blocks. This enables the entity to be pushable on contact, like tanks or
     * minecarts.
     */
    @Override
	...
	if (worldObj.isRemote && field_70279_a) {
		...
		d4 = posX + (tankX - posX) / tankPosRotationIncrements;
		d5 = posY + (tankY - posY) / tankPosRotationIncrements;
		d11 = posZ + (tankZ - posZ) / tankPosRotationIncrements;
		d10 = MathHelper.wrapAngleTo180_double(tankYaw - rotationYaw);
		rotationYaw = (float) (rotationYaw + d10
			/ tankPosRotationIncrements);
		rotationPitch = (float) (rotationPitch + (tankPitch - rotationPitch)
			/ tankPosRotationIncrements);
		--tankPosRotationIncrements;
		...
		if (list != null && !list.isEmpty())
			...
			if (entity != riddenByEntity && entity.canBePushed()
				&& entity instanceof EntitySteamTank)
    ...
    public void setPositionAndRotation2(final double par1, final double par3,
	    ...
	    final int par9) {
	if (field_70279_a)
	    tankPosRotationIncrements = par9 + 5;
	else {
