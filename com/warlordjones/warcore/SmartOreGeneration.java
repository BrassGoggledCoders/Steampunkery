...
public class SmartOreGeneration extends WorldGenerator {
    ...
    public SmartOreGeneration(final int par1, final int par2, final int par3, final int par4, final int par5) {
	minableBlockId = par1;
	metadata = par2;
	numberOfBlocks = par3;
	rarity = par4;
	...
	final double d1 = par3 + 8 - MathHelper.sin(f) * numberOfBlocks / 8F;
	final double d2 = par5 + 8 + MathHelper.cos(f) * numberOfBlocks / 8F;
	final double d3 = par5 + 8 - MathHelper.cos(f) * numberOfBlocks / 8F;
	...
	for (int i = 0; i <= numberOfBlocks; i++) {
	    ...
	    final double d9 = par2Random.nextDouble() * numberOfBlocks / 16D;
